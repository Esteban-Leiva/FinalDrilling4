...
class Vehiculo():
    ...
    def leer_datos_csv(self):
        try: 
            with open("vehiculos.csv") as archivo_csv:
                lector_csv = csv.reader(archivo_csv, delimiter = ",")

                tipo_vehiculo = str(type(self).__name__)
                print("Lista de Vehiculos",tipo_vehiculo)
                for linea in lector_csv:
                    if tipo_vehiculo in linea[0]:
                        print (f"{linea[1]}")
